...
wget -qOgit.tar.gz -o/dev/null \
	  'https://mirrors.edge.kernel.org/pub/software/scm/git/git-2.38.1.tar.gz';
/bin/echo 'done.';
...
wget -qOlzlib.tar.gz -o/dev/null \
	  https://download.savannah.gnu.org/releases/lzip/lzlib/lzlib-1.13.tar.gz;
/bin/echo 'done.';
/bin/echo -n 'Unpacking the tarball... ';
tar -xf lzlib.tar.gz;
...
wget -qOplzip.tar.gz -o/dev/null \
	  https://download.savannah.gnu.org/releases/lzip/plzip/plzip-1.10.tar.gz;
/bin/echo 'done.';
/bin/echo -n 'Unpacking the tarball... ';
tar -xf plzip.tar.gz;
...
wget -qOtarlz.tar.lz -o/dev/null \
	  https://download.savannah.gnu.org/releases/lzip/tarlz/tarlz-0.23.tar.lz;
/bin/echo 'done.';
/bin/echo -n 'Unpacking the tarball... ';
...
if uname -a | grep -q 'Debian'; then
	...
	wget -qOsource-code-pro.zip -o/dev/null \
	...
	wget -qOsource-sans.zip -o/dev/null \
	...
	wget -qOsource-serif.zip -o/dev/null \
	...
	wget -qO/usr/local/share/fonts/alabaster/SourceHanMono.ttc -o/dev/null \
	...
	wget -qOsource-han-sans.zip -o/dev/null \
	...
	wget -qOsource-han-serif.zip -o/dev/null \
		  'https://github.com/adobe-fonts/source-han-serif/releases/download/2.001R/01_SourceHanSerif.ttc.zip';
	...
	/bin/echo -e 'feh --bg-tile /usr/local/share/wallpapers/default.png &\ntint2 &' > /etc/xdg/openbox/autostart;
	/bin/echo 'done.';
