...
window.addEventListener("load", function() {
  ...
  {
    var w = window.openDialog(url, "_blank", "all=no,scrollbars=yes,resizable=yes,dialog=no,centerscreen");
  }
  ...
  function doSetDebuggingPrefs(v)
  {
	...
	Services.prefs.setBoolPref(debuggingPrefs[i], v);    
    }
    catch(e) {}
  }
  ...
  function doRefreshAllSkins()
  {
    try {
      // assuming bug 256504 makes it in, this should work with jar files
      Components.classes["@mozilla.org/chrome/chrome-registry;1"].getService(Components.interfaces.nsIXULChromeRegistry).refreshSkins();
    } catch(e) { alert(e) }
  }
  ...
  extDevExt.doRefreshAllSkins = doRefreshAllSkins;
